package com.myfin.views.contas;

import com.myfin.data.entity.SampleBook;
import com.myfin.data.service.SampleBookService;
...
@PageTitle("Contas")
@Route(value = "contas/:sampleBookID?/:action?(edit)", layout = MainLayout.class)
@PermitAll
public class ContasView extends Div implements BeforeEnterObserver {

    private final String SAMPLEBOOK_ID = "sampleBookID";
    private final String SAMPLEBOOK_EDIT_ROUTE_TEMPLATE = "contas/%s/edit";
    ...
    @Autowired
    public ContasView(SampleBookService sampleBookService) {
        this.sampleBookService = sampleBookService;
        addClassNames("contas-view");
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(ContasView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (sampleBookId.isPresent()) {
            ...
            if (sampleBookFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(ContasView.class);
            }
        }
    }
